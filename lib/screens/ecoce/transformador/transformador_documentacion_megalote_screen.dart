  ...
  Future<void> _loadTransformacion() async {
    try {
      ...
      if (doc.exists && mounted) {
        setState(() {
          _transformacion = TransformacionModel.fromFirestore(doc);
        });
      }
    } catch (e) {
      print('Error loading transformacion: $e');
    }
  }
  ...
  void _confirmarDocumentacion() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('Documentación cargada exitosamente. El megalote ha sido completado.'),
        ...
        // Navigate back to production screen with completed tab
        if (mounted) {
          Navigator.of(context).pushNamedAndRemoveUntil(
            '/transformador_produccion',
            (route) => false,
            arguments: {
              'initialTab': 2, // Completed tab
              'showMegalotes': true,
            },
          );
        }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'Megalote ${widget.transformacionId.substring(0, 8)}',
                              style: const TextStyle(
                                fontSize: 16,
                      ...
                      Text(
                        '${_transformacion!.lotesEntrada.length} lotes agrupados',
                        style: TextStyle(
                          fontSize: 14,
                      ...
                      Text(
                        'Peso total: ${_transformacion!.pesoTotalEntrada.toStringAsFixed(2)} kg',
                        style: TextStyle(
                          fontSize: 14,
                          color: Colors.grey[700],
                        ),
              ...
              const SizedBox(height: 8),
              Text(
                'Carga los documentos relacionados con el proceso de transformación del megalote.',
                style: TextStyle(
                  fontSize: 14,
                  color: Colors.grey[600],
                ),
